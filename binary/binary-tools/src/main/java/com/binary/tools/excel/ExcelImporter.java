...
public class ExcelImporter extends AbstractExcel {
	...
	private Object getCellValue(HSSFCell cell, Column col, boolean ismap, int i, int j, ImportListener listener) {
	...
	public <T> List<T> read(InputStream excel, Class<T> c, int fieldrow, int enumsrow, int startrow, ImportListener listener) {
		try {
			...
			POIFSFileSystem pfs = new POIFSFileSystem(excel);
